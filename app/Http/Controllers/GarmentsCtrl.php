...
class GarmentsCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $counts = count($type) + count($location) + count($keywords);
            ...
            if ($request->submit) {
                ...
                ->when($request->keywords,function($query)use($keywords,$lang){
                    $query->where(function($qry)use($keywords,$lang){
                        ...
                        //     // $qry->orWhere('company.name_jp','like',"%$keywords[$i]");
                            ...
                            $qry->orWhere("company.more_$lang",'like',"$keywords[$i]%");
                        }
                    });
                })
                ...
                ->when($request->service,function($query)use($service){$query->leftJoin('cp_service as sev','company.id','=','sev._id')->whereIn('sev.service',$service);})
