...
class LogisticsCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            ->where('company.industry',$industryId) 
            ->when($request->domestic,function($query)use($domestic){
                return $query->join('domestic as dmt','company.id','=','dmt._id')
                    ->where('dmt.transport','=',$domestic)
                    ->havingRaw('COUNT(dmt.id) >= ?',[1]);
            })
            ->when($request->international,function($query)use($international){
                $naLength = count($international);
                return $query->leftJoin('international as int','int._id','=','company.id')
                    ->whereIn('int.transport',$international)
                    ->havingRaw('COUNT(int.id) >= ?',[$naLength]);
            })
            ->when($request->methods,function($query)use($methods){
                $mtLength = count($methods);
                return $query->leftJoin('cp_method as met','met._id','=','company.id')
                    ->whereIn('met.method',$methods)
                    ->havingRaw('COUNT(met.id) >= ?',[$mtLength]);
            })
            ->when($request->warehouse,function($query)use($warehouse){
                $whLength = count($warehouse);
                return $query
                    ->whereHas('warehouse',function($sub)use($warehouse,$whLength){
                        $sub->whereIn('warehouse',$warehouse)
                            ->havingRaw('COUNT(id) = ?',[$whLength]);
                    });
            })
            ->when($request->services,function($query)use($services){
                $svLength = count($services);
            ...
            ->when($request->item,function($query)use($item){
                $imLength = count($item);
                return $query->leftJoin('cp_item as itm','itm._id','=','company.id')
                    ->whereIn('itm.item',$item)
                    ->havingRaw('COUNT(itm.id) >= ?',[$imLength]);
            })
    ...
    */
        ...
        switch ($request->step) {
            ...
            if (is_countable(@$request->domestics)>0) {
                if ($request->domestics) {
                    $new['domestic'] = new \App\Models\CpDomesticMd;
            ...
            $internationalMd = \App\Models\CpInternationalMd::class;
            ...
            $methodMd = \App\Models\CpMethodMd::class;
            ...
            $itemMd = \App\Models\CpItemMd::class;
            ...
            $serviceMd = \App\Models\CpServiceMd::class;
            ...
            $warehouseMd = \App\Models\CpWarehouseMd::class;
    ...
    public function filters($cid)
    {
        ...
        $warehouse = \App\Models\CpWarehouseMd::select("pro.province_name_$langP as province")->leftJoin('provinces as pro','cp_warehouse.warehouse','=','pro.province_id')->where(['cp_warehouse._id'=>@$cid]);
        return [
            'domestic' => $domestic,
            'logistics' => $logistics,
            'methods' => $methods,
    ...
    public function filtersHtml($cid)
    {
        ...
        if (@$filters['domestic']->transport) {
            ...
            $html.='<div class="col-lg-12"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.domestic").'</h5></div>';
            $html.='</div>';
        }
        if ($filters['logistics']->count() > 0) {
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.international").'</h5></div>';
        ...
        if ($filters['methods']->count() > 0) {
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.methods").'</h5></div>';
            $html.='<div class="col-lg-8"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
        ...
        if (@$filters['items']->count() > 0) {
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.items").'</h5></div>';
        ...
        if ($filters['services']->count()>0){
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.services").'</h5></div>';
            $html.='<div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
        ...
        if ($filters['warehouse']->count() > 0) {
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.warehouse").'</h5></div>';
            $html.='<div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
    ...
    public function cp(Request $request, $id=null)
    {
        ...
        ->leftJoin('district as dt','company.district','=','dt.district_id')
        ->leftJoin('sub-district as sd','company.subdistrict','=','sd.subdist_id')
        ->where(['company.id'=>$id,'industry'=>$this->industryId(),'public'=>1])
        ->first();
