...
class MachinesFoldingCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $bendingMachine = array_filter(explode(',',$request->get('bending-machine')));
            $foldingMachine = array_filter(explode(',',$request->get('folding-machine')));
            $materials = array_filter(explode(',',$request->materials));
            $service = array_filter(explode(',',$request->service));
            $distribute = $request->distribute;
            $location = array_filter(explode(',',$request->location));
            $count = count($bendingMachine)
                + count($foldingMachine)
                + count($materials)
                + count($service)
                + count($location);
            ...
            ->when($request->get('bending-machine'),function($query)use($bendingMachine){ 
                $length = count($bendingMachine);
                return $query
                    ->leftJoin('cp_product as cpp','company.id','=','cpp._id')
                    ->where('cpp.type','bending-machine')
                    ->whereIn('cpp._type',$bendingMachine)
                    ->havingRaw('COUNT(cpp.id) >= ?',[$length]);
            })
            ->when($request->get('folding-machine'),function($query)use($foldingMachine){ 
                $length = count($foldingMachine);
                return $query
                    ->leftJoin('cp_product as cpp2','company.id','=','cpp2._id')
                    ->where('cpp2.type','folding-machine')
                    ->whereIn('cpp2.product',$foldingMachine)
                    ->havingRaw('COUNT(cpp2.id) >= ?',[$length]);
            })
            ->when($request->materials,function($query)use($materials){ 
                $length = count($materials);
                return $query
                    ->leftJoin('cp_material as cpm','company.id','=','cpm._id')
                    ->whereIn('cpm.material',$materials)
