...
class CustomeBeautyCtrl extends Controller
{
    public static function index($request)
    {
        try {
            ...
            $count = count($costume) + count($accessories) + count($beauty) + count($location);
            ...
            ->when($request->beauty,function($query) use($beauty){
                $length = count($beauty); 
                return $query->leftJoin('cp_product as cpp','company.id','=','cpp._id')
                    ->where('cpp.type','beauty')
                    ->whereIn('cpp.product',$beauty)
                    ->havingRaw('COUNT(cpt.id) >= ?',[$length]); 
            })
