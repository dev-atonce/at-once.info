...
class PowerGenertionCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $agreement = array_filter(explode(',',$request->agreement));
            $type = array_filter(explode(',',$request->type));
            $turbine = $request->turbine;
            $count = count($location) + count($type) + count($agreement) + count($service);
            if($request->turbine) $count++;
            ...
            ->when($request->type, function($query) use($type){ 
                ...
                return $query->leftJoin('cp_type as cpt','company.id','=','cpt._id')
                    ->where('cp.type','type-of-medication')
            ...
            ->when($request->agreement, function($query) use($agreement){ 
                $length = count($agreement);
                return $query->leftJoin('cp_condition as cpd','company.id','=','cpd._id')
                    ->whereIn('cpd.condition',$condition)
                    ->havingRaw('COUNT(cpd.id) >= ?',[$length]); 
            })
            ->when($request->turbine, function($query) use($turbine){ 
                $length = count($turbine);
                return $query->leftJoin('cp_manufactor as cpm','company.id','=','cpm._id')
                    ->whereIn('cpm.manufactor',1)
