...
class CreditLoanCtrl extends Controller
{
    ...
    public static function index($request)
    {
        ...
        ->when($request->keywords,function($query)use($keywords,$request,$location,$categoryId){
            return $query
                ...
                ->when($request->location,function($sub)use($request,$location){
    ...
    public function cp($id=null)
    {
        ...
        $type = \App\Models\Filter\CpServiceMd::select(["ch.name_$lang as name"])->leftJoin('choice as ch','cp_service.service','=','ch.key')->where(['cp_service._id'=>$cp->id,'cp_service.type'=>'leasing','ch.type'=>'leasing-type']);
