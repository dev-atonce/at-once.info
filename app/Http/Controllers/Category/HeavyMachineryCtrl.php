...
class HeavyMachineryCtrl extends Controller
{
    ...
    public static function index($request)
    {
        ...
        ->where(['company.category'=>$categoryId ,'company.public'=>1]);
        ...
        if($category == 'heavy-machinery')
        {
            ...
            $location = array_filter(explode(',',$request->location));
            $count = count($location)+count($type)+count($service)+count($rental);
        ...
        {
            $type = array_filter(explode(',',$request->type));
            $fuelSystem = array_filter(explode(',',$request->get('fuel-system')));
            $location = array_filter(explode(',',$request->location));
            $count = count($type)+count($fuelSystem)+count($location);
    ...
    public function filtersHtml($cid)
    {
        ...
        if ($filters['rental']->count() > 0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->category.filter.rental").'</h5></div><div class="col-lg-10">';
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
