...
class LawFirmCtrl extends Controller
{
    ...
    public static function index($request)
    {
        ...
        ->when($request->keywords,function($query)use($keywords, $categoryId){
            ...
            return $query->where(function($query)use($keywords,$categoryId){
                return $query
        ...
        ->when($request->type,function($query)use($type){ 
            $length = count($type);
            return $query->leftJoin('cp_type as type','company.id','=','type._id')
                ->whereIn('type._type',$type)
                ->havingRaw('COUNT(type.id) >= ?',[$length]); 
        })
    ...
    public function filtersHtml($cid=null)
    {
        ...
        if ($filters['other']->count()>0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->category.filter.other").'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
        ...
        if ($filters['language']->count()>0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->category.filter.language").'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
