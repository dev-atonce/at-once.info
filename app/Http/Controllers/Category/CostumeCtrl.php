...
class CostumeCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $data['count'] = count($product) + count($location) + count($service);
            ...
            ->when($request->product, function($query) use($product){
                ...
                return $query->leftJoin('cp_product as cpt','company.id','=','cpt._id')
                    ->whereIn('cpt.product',$product)
