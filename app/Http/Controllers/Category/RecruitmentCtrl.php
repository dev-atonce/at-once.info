...
class RecruitmentCtrl extends Controller
{
    ...
    public static function index($request)
    {
        ...
        $count = count($position)+count($nationality)+count($employment)+count($location);
    ...
    public function filtersHtml($cid)
    {
        ...
        if ($filters['position']->count() > 0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->category.filter.position").'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
        ...
        if(@$filters['nationality']->count()>0){
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->category.filter.nationality").'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
        ...
        if(@$filters['type']->count()>0){
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->category.filter.employment").'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
    ...
    public function cp($id=null)
    {
        ...
        $position = \App\Models\Filter\CpPositionMd::select(["pos.position_$lang as name"])->leftJoin('job_position as pos','cp_position.position','=','pos.id')->where(['cp_position._id'=>$cp->id]);
        $nationality = \App\Models\Filter\CpNationalityMd::select(["ch.name_$lang as name"])->leftJoin('choice as ch','cp_nationality.nationality','=','ch.key')->where(['cp_nationality._id'=>$cp->id,'ch.type'=>'recruitment-nationality']);
        $type = \App\Models\Filter\CpTypeMd::select(["ch.name_$lang as name"])->leftJoin('choice as ch','cp_type._type','=','ch.key')->where(['cp_type._id'=>$cp->id,'ch.type'=>'type-recruitment']);
        $location = \App\Models\Filter\CpLocationMd::select(["pv.province_name_$lang as name"])->leftJoin('provinces as pv','cp_location.location','=','pv.province_id')->where(['cp_location._id'=>$cp->id]);
