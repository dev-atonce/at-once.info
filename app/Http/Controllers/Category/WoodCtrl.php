...
class WoodCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $wood = array_filter(explode(',',$request->wood));
            $product = array_filter(explode(',',$request->product));
            $count = count($location) + count($service) + count($wood) + count($product);
            ...
            ->when($request->wood, function($query) use($wood){ 
                $length = count($wood);
                return $query->leftJoin('cp_material as cpm','company.id','=','cpm._id')
                    ->whereIn('cpm.material',$wood)
