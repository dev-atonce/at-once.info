...
class MeasuringToolsCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            ->when($request->kind,function($query)use($kind){ 
                $length = count($kind);
                return $query
                    ->leftJoin('cp_type as cpt','company.id','=','cpt._id')
                    ->where("cp_type.type","kind-of-measuring-tool")
                    ->whereIn('cpt._type',$kind)
            ...
            ->when($request->type,function($query)use($type){ 
                ...
                return $query
                    ...
                    ->where("cpt2.type","type-of-measuring-tool")
                    ->whereIn('cpt2._type',$type)
