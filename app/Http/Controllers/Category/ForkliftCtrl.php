...
class ForkliftCtrl extends Controller
{
    ...
    public static function index($request)
    {
        ...
        {
            $type = array_filter(explode(',',$request->type));
            $fuelSystem = array_filter(explode(',',$request->get('fuel-sysytem')));
            $location = array_filter(explode(',',$request->location));
            $count = count($type)
                + count($fuelSystem)
                + count($location);
            ...
            })->when($request->get('fuel-system'),function($query) use($fuelSystem){ 
                $length = count($fuelSystem);
                return $query->leftJoin('cp_fuel as cpf','company.id','=','cpf._id')
                    ->whereIn('cpf.fuel',$fuelSystem)
                    ->havingRaw('COUNT(cpf.id) >= ?',[$length]); 
        ...
        $data['rows']->when($request->keywords,function($query)use($keywords, $categoryId){
            return $query
    ...
    public function filtersHtml($cid=null)
    {
        ...
        if ($filters['type']->count() > 0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->category.filter.type").'</h5></div><div class="col-lg-10">';
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['type']->get() as $k => $v){  $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';  }
            $html.='</div></li></ul></div></div>';
        }
        if ($filters['fuel']->count() > 0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->category.filter.fuel").'</h5></div><div class="col-lg-10">';
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
    ...
    public function cp($id=null)
    {
        ...
        // $service = \App\Models\Filter\CpServiceMd::select(["ch.name_$lang as name"])->leftJoin('choice as ch','cp_service.service','=','ch.key')->where(['cp_service._id'=>$cp->id,'ch.type'=>'forklift-service']);
        // $type = \App\Models\Filter\CpTypeMd::select(["ch.name_$lang as name"])->leftJoin('choice as ch','cp_type._type','=','ch.key')->where(['cp_type._id'=>$cp->id,'ch.type'=>'forklift-type']);
        // $fuel = \App\Models\Filter\CpTypeMd::select(["ch.name_$lang as name"])->leftJoin('choice as ch','cp_type._type','=','ch.key')->where(['cp_type._id'=>$cp->id,'ch.type'=>'fuel-system']);
        // $location = \App\Models\Filter\CpLocationMd::select("pro.province_name_$langP as name")->leftJoin('provinces as pro','cp_location.location','=','pro.province_id')->where(['cp_location._id'=>@$cp->id]);
