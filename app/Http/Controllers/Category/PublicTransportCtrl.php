...
class PublicTransportCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $pickUpPoint = array_filter(explode(',',$request->get('pick-up-point')));
            $destination = array_filter(explode(',',$request->destination));

            $count = count($location) + count($type) + count($pickUpPoint) + count($destination);
            ...
            ->when($request->get('pick-up-point'), function($query) use($pickUpPoint){
                $length = count($pickUpPoint);
                return $query->whereHas('location', function($sub) use($pickUpPoint, $length){
                    $sub->where('type','pick-up-point')
                        ->whereIn('location',$pickUpPoint)
                        ->havingRaw('COUNT(id) >= ?',[$length]);
                });
            })
            ->when($request->destination, function($query) use($destination){
                $length = count($destination);
                return $query->whereHas('location', function($sub) use($destination, $length){
                    $sub->where('type','destination')
                        ->whereIn('location',$destination)
                        ->havingRaw('COUNT(id) >= ?',[$length]);
                });
            })
