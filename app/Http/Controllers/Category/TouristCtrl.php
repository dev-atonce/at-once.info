...
class TouristCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $attractions = array_filter(explode(',',$request->attractions));
            $service = array_filter(explode(',',$request->service));
            $hikingCamping = $request->get('hiking-camping');
            $location = array_filter(explode(',',$request->location));
            $keywords = $request->keywords;

            $data['count'] = count($attractions) + count($service) + count($location);
            if($request->get('hiking-camping')) $data['count']++;
            ...
            ->when($request->attractions, function($query) use($attractions){
                $length = count($attractions);
                return $query->leftJoin('cp_type as cpt','company.id','=','cpt._id')
                    ->whereIn('cpt._type',$attractions)
            ...
            ->when($request->get('hiking-camping'),function($query)use($hikingCamping){
