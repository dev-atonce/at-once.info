...
class PackagingCtrl extends Controller
{
    ...
    public static function index($request)
    {
        ...
        $packaging = array_filter(explode(',',$request->packaging));
        $service = array_filter(explode(',',$request->service));
        $location = array_filter(explode(',',$request->location));
        $count = count($type) + count($material) + count($packaging) + count($service) + count($location);
        ...
        ->when($request->packaging,function($query)use($packaging){
            $length = count($packaging);
            return $query->join('cp_packaging as cpp','company.id','=','cpp._id')
                ->whereIn('cpp.packaging',$packaging)
                ->havingRaw('COUNT(cpp.id) >= ?',[$length]);
        })
        ->when($request->type, function($query)use($type){ 
            $length = count($type);
            return $query->join('cp_type as cpt','company.id','=','cpt._id')
        ...
        ->when($request->material, function($query) use($material){ 
            ...
            return $query->join('cp_material as cpm','company.id','=','cpm._id')
        ...
        ->when($request->service, function($query )use($service){ 
            ...
            return $query->join('cp_service as cps','company.id','=','cps._id')
