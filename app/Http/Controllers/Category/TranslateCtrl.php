...
class TranslateCtrl extends Controller
{
    ...
    public static function index($request)
    {
        try {
            ...
            $data['rows'] = \App\Models\CompanyMd::
                ...
                ->where(['company.public'=>1,'category'=> $categoryId])
    ...
    public function filters($cid)
    {
        $lang = Session('lang');
        // $langP = ($lang=='th')?'th':'en';
        $translate = \App\Models\Filter\CpTranslateMd::select('ch.id as key',"ch.name_$lang as name")->leftJoin('translate as ch','cp_translate.translate','=','ch.id')->where(['cp_translate._id'=>@$cid]);
        $speciality = \App\Models\Filter\CpSpecialityMd::select('ch.id as key',"ch.name_$lang as name")->leftJoin('speciality as ch','cp_speciality.speciality','=','ch.id')->where(['cp_speciality._id'=>@$cid]);
        $status = \App\Models\Filter\CpStatusMd::select('ch.id as key',"ch.name_$lang as name")->leftJoin('status as ch','cp_status.status','=','ch.id')->where(['cp_status._id'=>$cid]);
        $urgent = \App\Models\Filter\CpUrgentMd::where('_id',$cid)->select('urgent')->first();
        $postpay = \App\Models\Filter\CpPostpayMd::where('_id',$cid)->select('postpay')->first();
        return [
            'translate' => $translate,
    ...
    public function filtersHtml($cid)
    {
        ...
        if ($filters['translate']->count() > 0) {
            ...
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->category.filter.language").'</h5></div>';
        ...
        if(@$filters['speciality']->count()>0){
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->category.filter.speciality").'</h5></div>';
            $html.='<div class="col-lg-8"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
        ...
        if($filters['urgent']->urgent>0){
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-12"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->category.filter.urgent").'</h5></div>';
            $html.='</div>';
        }
        if($filters['postpay']->postpay>0){
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-12"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->category.filter.postpay").'</h5></div>';
            $html.='</div>';
        }
        if(@$filters['status']->count()>0){
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->category.filter.status").'</h5></div>';
            $html.='<div class="col-lg-8"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
