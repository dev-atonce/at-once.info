...
class AgriculturalChemicalsCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $organic = array_filter(explode(',',$request->organic));
            $chemical = array_filter(explode(',',$request->chemical));

            $count = count($location) + count($organic) + count($chemical);
            ...
            ->when($request->organic, function($query) use($organic){
                $length = count($organic);
                return $query->leftJoin('cp_type as cpt','company.id','=','cpt._id')
                    ->where('cpt.type','tool-for-plant')
                    ->whereIn('cpt._type',$organic)
                    ->havingRaw('COUNT(cpt.id) >= ?',[$length]); 
            })
            ->when($request->chemical, function($query) use($chemical){
                $length = count($chemical);
                return $query->leftJoin('cp_type as cpt2','company.id','=','cpt2._id')
                    ->where('cpt2.type','tools-for-moving-providing-water')
                    ->whereIn('cpt2._type',$chemical)
