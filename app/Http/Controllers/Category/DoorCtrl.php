...
class DoorCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $window = array_filter(explode(',',$request->window));
            $door = array_filter(explode(',',$request->door));
            ...
            ->when($request->window, function($query) use($window){
                $length = count($window);
                return $query->leftJoin('cp_material as cpp','company.id','=','cpp._id')
                    ->where('cpp.type','type-of-window')
                    ->whereIn('cpp._type',$window)
                    ->havingRaw('COUNT(cpp.id) >= ?',[$length]);
            })
            ->when($request->door, function($query) use($door){
                $length = count($door);
                return $query->leftJoin('cp_product as cpp2','company.id','=','cpp2._id')
                    ->where('cpp2.type','type-of-door')
                    ->whereIn('cpp2._type',$door)
