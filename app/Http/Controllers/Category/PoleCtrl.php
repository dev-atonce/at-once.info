...
class PoleCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $cross = array_filter(explode(',',$request->cross));
            ...
            ->when($request->type, function($query) use($type){
                ...
                return $query->leftJoin('cp_type as cpt','company.id','=','cpt._id')
                    ->where('cpt.type','type-of-mast')
                    ->whereIn('cpt._type',$type)
                    ->havingRaw('COUNT(cpt.id) >= ?',[$length]);
            })
            ->when($request->cross, function($query) use($cross){
                $length = count($cross);
                return $query->leftJoin('cp_type as cpt2','company.id','=','cpt2._id')
                    ->where('cpt2.type','cross-type')
                    ->whereIn('cpt2._type',$cross)
