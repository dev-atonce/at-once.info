...
class MachineryRepairCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $workPattern = array_filter(explode(',',$request->get('work-pattern')));
            $overhual = $request->overhual;
            $location = array_filter(explode(',',$request->location));
            $count = count($location) + count($type) + count($workPattern);
            if($request->overhual) $count = $count + 1;
            ...
            ->when($request->type,function($query)use($type){ 
                ...
                return $query
                    ...
                    ->whereIn('cpp.product',$service)
                    ->havingRaw('COUNT(cpp.id) >= ?',[$length]);
            })
            ->when($request->get('work-pattern'),function($query)use($workPattern){ 
                $length = count($workPattern);
                return $query
                    ->leftJoin('cp_type as cpt','company.id','=','cpt._id')
                    ->whereIn('cpt.type',$workPattern)
                    ->havingRaw('COUNT(cpt.id) >= ?',[$length]);
            })
            ->when($request->overhual,function($query)use($overhual){ 
                $length = count($overhual);
                return $query
                    ->leftJoin('cp_overhual as cpo','company.id','=','cpo._id')
                    ->where('cpo.overhual',1);
            })
