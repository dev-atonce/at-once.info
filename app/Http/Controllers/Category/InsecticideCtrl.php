...
class InsecticideCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $serviceLocation = array_filter(explode(',',$request->get('service-location')));
            $location = array_filter(explode(',',$request->location));
            $count = count($location)+ count($serviceLocation) + count($service);
            $keywords = $request->keywords;

            $svLength = count($service) + count($location);
            $data['count'] = $count;
            ...
            ->when($request->service,function($query)use($service){ 
                ...
                return $query
                    ...
                    ->where('cps.type',"insecticide-service")
                    ->whereIn('cps.service',$service)
                    ->havingRaw('COUNT(cps.id) >= ?',[$length]);
            })
            ->when($request->get('service-location'),function($query)use($serviceLocation){ 
                $length = count($serviceLocation);
                return $query
                    ->leftJoin('cp_service as cps2','company.id','=','cps2._id')
                    ->where('cps.type',"insecticide-site")
                    ->whereIn('cps2.service',$serviceLocation)
