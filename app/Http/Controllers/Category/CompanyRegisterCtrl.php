...
class CompanyRegisterCtrl extends Controller
{
    ...
    public static function index($request)
    {
        ...
        $count = count($location)+count($consulting)+count($service);
    ...
    public function store(Request $request)
    {
        ...
        switch ($request->step) {
            ...
            case 2:
                ...
                $CpConsultingMd = \App\Models\Filter\CpConsultingMd::class;
    ...
    public function filters($cid)
    {
        ...
        $consulting = \App\Models\Filter\CpConsultingMd::select('ch.id',"ch.name_$lang as name")->leftJoin('consulting as ch','cp_consulting.consulting','=','ch.id')->where(['cp_consulting._id'=>@$cid]);
        $service = \App\Models\Filter\CpServiceMd::select('ch.id',"ch.name_$lang as name")->leftJoin('choice as ch','cp_service.service','=','ch.key')->where(['cp_service._id'=>$cid,'ch.type'=>'setting-service']);
        return [
            'location' => $location,
    ...
    public function filtersHtml($cid)
    {
        ...
        if ($filters['location']->count() > 0) {
            ...
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->category.filter.location").'</h5></div>';
        ...
        if ($filters['consulting']->count() > 0) {
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-3"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->category.filter.consulting").'</h5></div>';
        ...
        if ($filters['service']->count() > 0) {
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->category.filter.service").'</h5></div>';
            $html.='<div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
    ...
    public function cp($id=null)
    {
        ...
        $consulting = \App\Models\Filter\CpConsultingMd::select('ch.id',"ch.name_$lang as name")->leftJoin('consulting as ch','cp_consulting.consulting','=','ch.id')->where(['cp_consulting._id'=>@$cp->id]);
        $service = \App\Models\Filter\CpServiceMd::select('ch.id',"ch.name_$lang as name")->leftJoin('choice as ch','cp_service.service','=','ch.key')->where(['cp_service._id'=>$cp->id,'ch.type'=>'setting-service']);
