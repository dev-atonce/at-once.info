...
class MachinesDressingCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $cutter = array_filter(explode(',',$request->cutter));
            $drilling = array_filter(explode(',',$request->drilling));
            $lathe = array_filter(explode(',',$request->lathe));
            $grinding = array_filter(explode(',',$request->grinding));
            ...
            ->when($request->cutter,function($query)use($cutter){ 
                $length = count($cutter);
                return $query
                    ->leftJoin('cp_product as cpp','company.id','=','cpp._id')
                    ->where('cpp.type','type-of-cutter')
                    ->whereIn('cpp._product',$cutter)
                    ->havingRaw('COUNT(cpp.id) >= ?',[$length]);
            })
            ->when($request->drilling,function($query)use($drilling){ 
                $length = count($drilling);
                return $query
                    ->leftJoin('cp_product as cpp2','company.id','=','cpp2._id')
                    ->where('cpp2.type','type-of-cutter')
                    ->whereIn('cpp2.product',$drilling)
                    ->havingRaw('COUNT(cpp2.id) >= ?',[$length]);
            })
            ->when($request->lathe,function($query)use($lathe){ 
                $length = count($lathe);
                return $query
                    ->leftJoin('cp_product as cpp3','company.id','=','cpp3._id')
                    ->where('cpp3.type','type-of-cutter')
                    ->whereIn('cpp3.product',$lathe)
                    ->havingRaw('COUNT(cpp3.id) >= ?',[$length]);
            })
            ->when($request->grinding,function($query)use($grinding){ 
                $length = count($grinding);
                return $query
                    ->leftJoin('cp_product as cpp4','company.id','=','cpp4._id')
                    ->where('cpp4.type','type-of-cutter')
                    ->whereIn('cpp4.product',$grinding)
