...
class BrickCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $tile = array_filter(explode(',',$request->tile));
            $location = array_filter(explode(',',$request->location));

            $count = count($location) + count($type) + count($tile);
            ...
            ->when($request->type, function($query) use($type){
                ...
                return $query->leftJoin('cp_product as cpp','company.id','=','cpp._id')
                    ->where('cpp.type','type-of-brick')
                    ->where('cpp.type','type-of-soil')
                    ->whereIn('cpp.service',$type)
                    ->havingRaw('COUNT(cpp.id) >= ?',[$length]);
            })
            ->when($request->tile, function($query) use($tile){
                $length = count($tile);
                return $query->leftJoin('cp_product as cpp2','company.id','=','cpp2._id')
                    ->where('cpp2.type','type-of-tile')
                    ->whereIn('cpp2.product',$tile)
