...
class MachinesStampingCtrl extends Controller
{
    public static function index($request)
    {
        try
        {
            ...
            $compression = array_filter(explode(',',$request->compression));
            ...
            ->when($request->compression,function($query)use($compression){ 
                $length = count($compression);
                return $query
                    ->leftJoin('cp_service as cps','company.id','=','cps._id')
                    ->where('cps.type','compression')
                    ->whereIn('cps.service',$compression)
