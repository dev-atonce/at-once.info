...
class AccountingCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $counts = count($location)+count($other)+count($service)+count($nationality);
        ...
        ->when($request->other,function($query)use($other){
            $length = count($other);
            return $query->leftJoin('cp_other as oh','company.id','=','oh._id')
                ->whereIn('oh.other',$other)
                ->havingRaw('COUNT(oh.id) >= ?',[$length]);
        })
        ...
        ->when($request->keywords,function($query)use($keywords){
            return $query
                ...
                ->having('public',1)
                ->having('industry',$this->industryId())
                ->groupBy('company.id');
        })
    ...
    public function informationUpdate(Request $request,$id=null)
    {
        try {
            ...
            * Domestic
            */
            $CpDomesticMd = \App\Models\CpDomesticMd::class;
            if($request->domestic){
            ...
            */
            $CpInternationalMd = \App\Models\CpInternationalMd::class;
            ...
            * Methods
            */
            $CpMethodMd = \App\Models\CpMethodMd::class;
            if($request->method){
            ...
            * Item
            */
            $CpItemMd = \App\Models\CpItemMd::class;
            if($request->item){
            ...
            * Service
            */
            $CpServiceMd = \App\Models\CpServiceMd::class;
            if($request->services){
            ...
            * Warehouse
            */
            $CpWarehouseMd = \App\Models\CpWarehouseMd::class;
    ...
    public function filtersHtml($cid)
    {
        ...
        if (@$filters['service']->count()>0){
            ...
            foreach(@$filters['service']->get() as $kl => $lo){
        ...
        if ($filters['other']->count()>0) {
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.other").'</h5></div>';
            $html.='<div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';     
            foreach(@$filters['other']->get() as $kv => $va){
        ...
        if ($filters['nationality']->count()>0) {
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.nationality").'</h5></div>';
            $html.='<div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';     
            foreach(@$filters['nationality']->get() as $kv => $va){
                $html.='<div class="pix1uw-0 ggGntR">'.$va->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
        if ($filters['location']->count()>0) {
            ...
            $html.='<div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';     
            foreach(@$filters['location']->get() as $kv => $va){
                $html.='<div class="pix1uw-0 ggGntR">'.$va->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
