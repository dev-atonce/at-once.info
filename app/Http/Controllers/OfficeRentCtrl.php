...
class OfficeRentCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $counts = count($location)+count($service);
        ...
        if ($request->submit) {
            ...
            ->when($request->service,function($query)use($service){$query->leftJoin('cp_service as ser','company.id','=','ser._id')->whereIn('ser.service',$service);})
    ...
    public function filtersHtml($cid=null)
    {
        ...
        if(@$filters['service']->count()>0){
            $html.='<div class="row"><div class="col-lg-3"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.service").'</h5></div><div class="col-lg-9">';
        ...
        if(@$filters['contract']->count()>0){
            $html.='<div class="row"><div class="col-lg-3"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.contract").'</h5></div><div class="col-lg-9">';
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['contract']->get() as $k => $v){
                $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
        if ($filters['location']->count() > 0) {
            $html.='<div class="row"><div class="col-lg-3"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.location").'</h5></div><div class="col-lg-9">';
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
