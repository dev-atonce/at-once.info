...
class ForkliftCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $counts = count($location)+count($service)+count($type)+count($fuel);
        ...
        ->when($request->type,function($query) use($type){
            ...
            return $query->leftJoin('cp_type as typ','company.id','=','typ._id')
                ->whereIn('typ._type',$type)
                ->havingRaw('COUNT(typ.id) >= ?',[$length]); 
        })
        ->when($request->fuel,function($query)use($fuel){ 
            $length = count($fuel); 
            return $query->leftJoin('cp_fuel as fue','company.id','=','fue._id')
                ->whereIn('fue.fuel',$fuel)
                ->havingRaw('COUNT(fue.id) >= ?',[$length]); 
        })
        ...
        $rows = $data->orderBy('company.type')->inRandomOrder()->limit(20)->get();
    ...
    public function filtersHtml($cid=null)
    {
        ...
        if ($filters['service']->count() > 0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.service").'</h5></div><div class="col-lg-10">';
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['service']->get() as $k => $v){  $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';  }
            $html.='</div></li></ul></div></div>';
        }
        if ($filters['type']->count() > 0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.type").'</h5></div><div class="col-lg-10">';
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['type']->get() as $k => $v){  $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';  }
            $html.='</div></li></ul></div></div>';
        }
        if ($filters['fuel']->count() > 0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.fuel").'</h5></div><div class="col-lg-10">';
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['fuel']->get() as $k => $v){  $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';  }
            $html.='</div></li></ul></div></div>';
        }
        if ($filters['location']->count() > 0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.location").'</h5></div><div class="col-lg-10">';
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['location']->get() as $k => $v){  $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';  }
            $html.='</div></li></ul></div></div>';
        }
        $html.='</div>';
        return $html;
    ...
    public function cp($id=null)
    {
        ...
        // $service = \App\Models\CpServiceMd::select(["ch.name_$lang as name"])->leftJoin('choice as ch','cp_service.service','=','ch.key')->where(['cp_service._id'=>$cp->id,'ch.type'=>'forklift-service']);
        // $type = \App\Models\CpTypeMd::select(["ch.name_$lang as name"])->leftJoin('choice as ch','cp_type._type','=','ch.key')->where(['cp_type._id'=>$cp->id,'ch.type'=>'forklift-type']);
        // $fuel = \App\Models\CpTypeMd::select(["ch.name_$lang as name"])->leftJoin('choice as ch','cp_type._type','=','ch.key')->where(['cp_type._id'=>$cp->id,'ch.type'=>'fuel-system']);
        // $location = \App\Models\CpLocationMd::select("pro.province_name_$langP as name")->leftJoin('provinces as pro','cp_location.location','=','pro.province_id')->where(['cp_location._id'=>@$cp->id]);
