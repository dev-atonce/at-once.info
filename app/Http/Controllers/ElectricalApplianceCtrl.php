...
class ElectricalApplianceCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $counts = count($type)+count($brand)+count($location);
            ...
            ->when($request->type,function($query)use($type){
                $length = count($type);
                //==================================//
                $query->leftJoin('cp_appliance as ap','company.id','=','ap._id')             
            ...
            ->when($request->brand,function($query)use($brand){
                ...
                //==================================//
                $query
                    ->leftJoin('cp_brand as br','company.id','=','br._id')
            ...
            $rows = $query->orderBy('company.type','desc')
                ->inRandomOrder()
                ->limit(20)
                ->get();
