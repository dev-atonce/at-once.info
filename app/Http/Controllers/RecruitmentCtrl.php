...
class RecruitmentCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $counts = count($position)+count($nationality)+count($employment)+count($location);
        ...
        ->when($request->position,function($query) use($position){
            ...
            return $query->leftJoin('cp_position as pos','company.id','=','pos._id')
                ->whereIn('pos.position',$position)
                ->havingRaw('COUNT(pos.id) >= ?',[$length]);
        })
        ->when($request->nationality,function($query) use($nationality){
            $length = count($nationality);
            return $query->leftJoin('cp_nationality as nat','company.id','=','nat._id')
                ->whereIn('nat.nationality',$nationality)
                ->havingRaw('COUNT(nat.id) >= ?',[$ntLength]);
        })
        ->when($request->employment,function($query) use($employment){
            $length = count($employment);
            return $query->leftJoin('cp_type as typ','company.id','=','typ._id')
                ->whereIn('typ._type',$employment)
                ->havingRaw('COUNT(typ.id) >= ?',[$length]);
        })
    ...
    public function statistics()
    {
        ...
        if(@$row->id)
            return view("$this->prefix.member.dashboard",[
                ...
                'module' => $this->industry,
                '_id' => Auth::guard('Members')->id(),
                'row' => \App\Models\CompanyMd::where(['_id'=>Auth::guard('Members')->id(),'industry'=>$this->industryId()])->select('id','name_jp','name_th','logo')->first()
            ]);
        else
    ...
    public function store(Request $request)
    {
        ...
        switch ($request->step) {
            ...
            case 2:
                ...
                $CpTranslateMd = \App\Models\CpTranslateMd::class;
                ...
                $CpSpecialityMd = \App\Models\CpSpecialityMd::class;
                ...
                if (is_countable(@$request->urgent)>0) {
                    $new['urgent'] = new \App\Models\CpUrgentMd;
                ...
                if (is_countable(@$request->postpay)>0) {
                    $new['postpay'] = new \App\Models\CpPostpayMd;
                ...
                if (is_countable(@$request->status)>0) {
                    $CpStatusMd = \App\Models\CpStatusMd::class;
    ...
    public function informationUpdate(Request $request,$id=null)
    {
        try {
            ...
            * Urgent
            */
            $CpUrgentMd = \App\Models\CpUrgentMd::class;
            if($request->urgent){
            ...
            * Postpay
            */
            $CpPostpayMd = \App\Models\CpPostpayMd::class;
            if($request->postpay){
    ...
    public function filtersHtml($cid)
    {
        ...
        if ($filters['position']->count() > 0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.position").'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['position']->get() as $kw => $wh){
                $html.='<div class="pix1uw-0 ggGntR">'.$wh->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
        if(@$filters['nationality']->count()>0){
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.nationality").'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['nationality']->get() as $kc => $vc){
                $html.='<div class="pix1uw-0 ggGntR">'.$vc->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
        if(@$filters['type']->count()>0){
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.employment").'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['type']->get() as $kc => $vc){
                $html.='<div class="pix1uw-0 ggGntR">'.$vc->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
        if(@$filters['location']->count()>0){
            ...
            foreach(@$filters['location']->get() as $kc => $vc){
                $html.='<div class="pix1uw-0 ggGntR">'.$vc->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
    ...
    public function cp($id=null)
    {
        ...
        $position = \App\Models\CpPositionMd::select(["pos.position_$lang as name"])->leftJoin('job_position as pos','cp_position.position','=','pos.id')->where(['cp_position._id'=>$cp->id]);
        $nationality = \App\Models\CpNationalityMd::select(["ch.name_$lang as name"])->leftJoin('choice as ch','cp_nationality.nationality','=','ch.key')->where(['cp_nationality._id'=>$cp->id,'ch.type'=>'recruitment-nationality']);
        $type = \App\Models\CpTypeMd::select(["ch.name_$lang as name"])->leftJoin('choice as ch','cp_type._type','=','ch.key')->where(['cp_type._id'=>$cp->id,'ch.type'=>'type-recruitment']);
        $location = \App\Models\CpLocationMd::select(["pv.province_name_$lang as name"])->leftJoin('provinces as pv','cp_location.location','=','pv.province_id')->where(['cp_location._id'=>$cp->id]);
