...
class CreditLoanCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $counts = count($location)+count($type);
        ...
        ->when($request->keywords,function($query)use($keywords,$request,$location){
            $query
                ->leftJoin('cp_location as lk','company.id','=','lk._id')
                ->leftJoin('provinces as pk','pk.province_id','=','lk.location');
            return $query->where(function($query)use($keywords){
        ...
        ->when($request->location,function($sub)use($location){
            $sub->whereHas('location',function($third)use($location){
                $length = count($location);
                return $third->whereIn('lk.location',$location)
                ->havingRaw('COUNT(lk.id) >= ?',[$length]);
            });
        })
        ->when($request->type, function($query) use($type){ 
            $length = count($type);
            return $query->leftJoin('cp_service as sv','company.id','=','sv._id')
                ->whereIn('sv.service',$type)
