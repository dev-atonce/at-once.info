...
class OfficeSuppliesCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        try
        {
            ...
            $query = \App\Models\CompanyMd::where([
                'company.public' => 1,
            ...
            $rows = $query->orderBy('company.type','desc')->inRandomOrder()->limit(20)->get();
    ...
    public function testQuery(Request $request)
    {
        ...
        $newQuery = \App\Models\CompanyMd::where([
            'company.type' => 'full',
            'company.public' => 1,
            'company.industry' => $this->industryId()
        ])        
        ->when($request->keywords,function($query)use($keywords){
            ...
            ->whereRaw('REPLACE(pv.province_name_th," ","") LIKE ?',["%".str_replace(' ','',$keywords)."%"])
            ->orWhereRaw('REPLACE(company.name_th," ","") LIKE ?',["%".str_replace(' ','',$keywords)."%"])
            ->orWhereRaw('REPLACE(company.description_th," ","") LIKE ?',["%".str_replace(' ','',$keywords)."%"])
            ->orWhereRaw('REPLACE(company.detail_jp," ","") LIKE ?',["%".str_replace(' ','',$keywords)."%"]);
        })
        ->when($request->type,function($query)use($type,$keywords){
            $length = count($type);
            //================ *** ===============//
            $query->leftJoin('cp_type as ty','company.id','=','ty._id')             
                ->whereIn('ty._type',$type)
                ->havingRaw('COUNT(ty.id) >= ?',[$length]);
        })
        ->when($request->location,function($query)use($location,$keywords){
            $length = count($location);
            //================ *** ===============//
            $query->leftJoin('cp_location as lt','company.id','=','lt._id')
        ...
        $online = $newQuery->get()->count();
        $rows = $newQuery->inRandomOrder()->limit(20)->get();


        $seo = \App\Helpers\SeoLandingPage::getIdustrySeoKeyword($this->industryId());


        return view("$this->prefix.$this->industry.index-test",[
            'prefix' => $this->prefix,
            'module' => $this->industry,
            'industryName' => $this->industryName(),
            'lang' => $lang,
    ...
    */
    ...
    public function create(Request $request)
    {
        switch ($request->step) {
            ...
            break;
            case 3:            return view("$this->prefix.member.first.step3",[
                'prefix' => $this->prefix,
                'module' => $this->industry,
                'industryId' => $this->industryId(),
                '_id' => Auth::guard('Members')->id()
