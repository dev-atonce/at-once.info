...
class ConstructionCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $other = array_filter(explode(',',$request->type));
        $service = array_filter(explode(',',$request->service));

        $counts = count($location)+count($other)+count($service);
        ...
        if ($request->submit) {
            $query = $data
            ->where(['company.category'=>$this->categoryId(),'company.public'=>1])
            ...
            ->when($request->other,function($query)use($other){ $query->leftJoin('cp_other as ot','company.id','=','ot._id')->whereIn('ot.other',$rental); })
            ->groupBy('company.id');

            $online = $query->get()->count();
            $rows = $query->orderBy('company.type')->inRandomOrder()->limit(20)->get();
        }else {
            $query = $data->where(['company.category'=>$this->categoryId(),'company.public'=>1]);
            $rows = $query->inRandomOrder()
                ->limit(20)
                ->get();
        }

        $online = \App\Http\Controllers\Api\CategoryCtrl::online($this->categoryId());

        $seo = \App\Helpers\SeoLandingPage::getCategorySeoKeyword($this->categoryId());

        return view("$this->prefix.$this->category.index",[
            'prefix' => $this->prefix,
    ...
    public function uploadGallery(Request $request)
    {
        ...
        if ($glImage) {
            ...
            $get = \App\Models\CompanyMd::where(['_id'=>$_id,'v'=>$this->vId()])->first();
    ...
    public function filtersHtml($cid)
    {
        ...
        if ($filters['other']->count() > 0) {
            ...
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['other']->get() as $k => $v){
                $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
