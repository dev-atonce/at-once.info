...
class LeasingCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $counts = count($location)+count($service)+count($type);
        ...
        if ($request->submit) {
            ...
            ->when($request->service,function($query)use($service){ $query->leftJoin('cp_service as ser','company.id','=','con._id')->whereIn('ser.service',$service); })
            ->when($request->type,function($query)use($type){ $query->leftJoin('cp_type as con','company.id','=','con._id')->whereIn('con.type',$type); })
    ...
    public function filtersHtml($cid=null)
    {
        ...
        if(@$filters['service']->count()>0){
            ...
            foreach(@$filters['service']->get() as $kc => $vc){ $html.='<div class="pix1uw-0 ggGntR">'.$vc->name.'</div>'; }
        ...
        if (@$filters['location']->count() > 0) {
            ...
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['location']->get() as $kw => $wh){ $html.='<div class="pix1uw-0 ggGntR">'.$wh->name.'</div>'; }
            $html.='</div></li></ul></div></div>';
        }
        $html.='</div>';
        return $html;
    ...
    public function cp($id=null)
    {
        ...
        $type = \App\Models\CpServiceMd::select(["ch.name_$lang as name"])->leftJoin('choice as ch','cp_service.service','=','ch.key')->where(['cp_service._id'=>$cp->id,'cp_service.type'=>'leasing','ch.type'=>'leasing-type']);
