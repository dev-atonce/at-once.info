...
class VisaCtrl extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        switch ($request->step) {
            ...
            case 2:
                ...
                // insert into table : cp_visa
                $CpVisaMd = \App\Models\CpVisaMd::class;
                if (is_countable(@$request->type)>0) {
                    foreach ($request->type as $ty) {
                        $new['type'] = new $CpVisaMd;
                        $new['type']->_id = $get->id;
                        $new['type']->type = $ty;
    ...
    public function informationUpdate(Request $request,$id=null)
    {
        try {
            ...
            */
            $CpLocationMd = \App\Models\CpLocationMd::class;
            if($request->location){
                foreach($request->location as $kin => $in) {
                    if($CpLocationMd::where(['_id'=>$_id,'location'=>$request->location])->count() < 1) {
                        $newD = new $CpLocationMd;
                        $newD->_id = $get->id;
                        $newD->location = $request->location;
                        $newD->save();
                    }
                }
                $CpLocationMd::where('_id',$get->id)->whereNotIn('location',$request->international)->delete();
            }else{
            ...
            */
            $CpVisaMd = \App\Models\CpVisaMd::class;
            if($request->type){
                foreach($request->type as $kty => $ty) {
                    if($CpVisaMd::where(['_id'=>$get->id,'visa'=>$ty])->count() == 0){
                        $CpVisaMd::insert(['_id'=>$get->id,'visa'=>$ty]);
                    }
                }
                $CpVisaMd::where('_id',$get->id)->whereNotIn('visa',$request->type)->delete();
            }else{
                $CpVisaMd::where('_id',$get->id)->delete();
            }
    ...
    public function filtersHtml($cid)
    {
        ...
        if ($filters['visa']->count()>0) {
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__('phrase.visa-type').'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['visa']->get() as $kv => $va){
    ...
    public function cp($id=null)
    {
        ...
        $visa = \App\Models\CpVisaMd::select("ch.name_$lang as name")->leftJoin('choice as ch','cp_visa.visa','=','ch.key')->where(['cp_visa._id'=>@$cp->id,'ch.type'=>'contract-period']);
