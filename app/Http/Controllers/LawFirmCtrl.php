...
class LawFirmCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $counts = count($location)+count($service)+count($other)+count($language);
        ...
        ->when($request->service,function($query)use($service){ 
            ...
            return $query->leftJoin('cp_service as serv','company.id','=','serv._id')
                ->whereIn('serv.service',$service)
                ->havingRaw('COUNT(serv.id) >= ?',[$length]); 
        })
        ->when($request->other,function($query)use($other){ 
            $length = count($other);
            return $query->leftJoin('cp_other as oth','company.id','=','oth._id')
                ->whereIn('oth.other',$other)
                ->havingRaw('COUNT(oth.id) >= ?',[$length]); 
        })
    ...
    public function filtersHtml($cid=null)
    {
        ...
        if ($filters['service']->count()>0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.service").'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
        ...
        if ($filters['other']->count()>0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.other").'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
        ...
        if ($filters['language']->count()>0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.language").'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['language']->get() as $kv => $va){
                $html.='<div class="pix1uw-0 ggGntR">'.$va->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
