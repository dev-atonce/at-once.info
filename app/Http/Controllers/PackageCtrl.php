...
class PackageCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $counts = count($location)+count($services)+count($other);
        $keywords = $request->keywords;

        $svLength = count($services);
        $otLength = count($other);
        ...
        ->when($request->services, function($query )use($services){ 
            ...
            return $query->leftJoin('cp_service as sv','company.id','=','sv._id')
                ->whereIn('sv.service',$services)
                ->havingRaw('COUNT(sv.id) >= ?',[$length]); 
        })
        ->when($request->other, function($query) use($other){ 
            $length = count($other);
            return $query->leftJoin('cp_other as ot','company.id','=','ot._id')
                ->whereIn('ot.other',$other)
                ->havingRaw('COUNT(ot.id) >= ?',[$length]); 
        })
    ...
    public function filtersHtml($cid=null)
    {
        ...
        if ($filters['service']->count() > 0) {
            ...
            foreach(@$filters['service']->get() as $ks => $vs){
                $html.='<div class="pix1uw-0 ggGntR">'.$vs->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
        if(@$filters['other']->count()>0){
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.other").'</h5></div><div class="col-lg-10">';
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
        ...
        if(@$filters['location']->count()>0){
            ...
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['location']->get() as $kl => $vl){
                $html.='<div class="pix1uw-0 ggGntR">'.$vl->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }  
        $html.='</div>';
