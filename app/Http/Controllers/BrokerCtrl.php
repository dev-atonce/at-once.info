...
class BrokerCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $counts = count($location)+count($service)+count($type)+count($nationality);
        $keywords = $request->keywords;

        $ltLength = count($location);
        $svLength = count($service);
        $tpLength = count($type);
        $naLength = count($nationality);

        $data = \App\Models\CompanyMd::where([
            'company.industry' => $this->industryId(),
            'company.public' => 1
        ])           
        ->when($request->service, function($query) use($service, $svLength){
            $length = count($service);
            return $query->join('cp_service as sev','company.id','=','sev._id')
                ->whereIn('sev.service',$service)
                ->havingRaw('COUNT(sev.id) >= ?',[$length]);
        })
        ->when($request->type, function($query) use($type, $tpLength){ 
            $length = count($type);
            return $query->join('cp_type as con','company.id','=','con._id')
                ->whereIn('con._type',$type)
                ->havingRaw('COUNT(con.id) >= ?',[$length]); 
        })
        ->when($request->nationality,function($query) use($nationality){ 
            $length = count($nationality);
            return $query->join('cp_nationality as nat','company.id','=','nat._id')
                ->whereIn('nat.nationality',$nationality)
                ->havingRaw('COUNT(nat.id) >= ?',[$length]); 
        })
        ->when($request->location, function($query) use($location, $ltLength){ 
    ...
    public function filtersHtml($cid=null)
    {
        ...
        if ($filters['service']->count() > 0) {
            ...
            foreach(@$filters['service']->get() as $ks => $ser){
                $html.='<div class="pix1uw-0 ggGntR">'.$ser->name.'</div>';
            }
        ...
        if(@$filters['nationality']->count()>0){
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.nationality").'</h5></div><div class="col-lg-10">';
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['nationality']->get() as $kn => $vn){
                $html.='<div class="pix1uw-0 ggGntR">'.$vn->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }  
        $html.='</div>';
