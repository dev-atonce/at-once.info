...
class InteriorCtrl extends Controller
{
        ...
        if ($request->submit) {
            ...
            ->when($request->keywords,function($query)use($keywords,$lang){
                $query->where(function($qry)use($keywords,$lang){
                    $qry->where("company.name_$lang",'like',"%$keywords[0]%"); 
                    for($i=0;$i<count($keywords);$i++){ 
                        $qry->orWhere("company.name_$lang",'like',"%$keywords[$i]%");
                        $qry->orWhere("company.description_$lang",'like',"%$keywords[$i]%");
                        $qry->orWhere("company.description_$lang",'like',"%$keywords[$i]%");
                        $qry->orWhere("company.detail_$lang",'like',"%$keywords[$i]%");
                        $qry->orWhere("company.detail_$lang",'like',"%$keywords[$i]%");
                    }
                });
            })
