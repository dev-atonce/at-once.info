...
class ConstructionMachineCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $counts = count($location)+count($type)+count($service)+count($rental);
        ...
        if ($request->submit) {
            ...
            ->when($request->keywords,function($query)use($keywords){
                $query->where(function($qry)use($keywords){
                    for($i=0;$i<count($keywords);$i++){ 
                        $qry->orWhere('company.name_th','like',"%".$keywords[$i]."%"); 
                        $qry->orWhere('company.name_jp','like',"%".$keywords[$i]."%");
                        $qry->orWhere('company.description_th','like',"%".$keywords[$i]."%");
                        $qry->orWhere('company.description_jp','like',"%".$keywords[$i]."%");
                        $qry->orWhere('company.detail_th','like',"%".$keywords[$i]."%");
                        $qry->orWhere('company.detail_jp','like',"%".$keywords[$i]."%");
                    }
                });
            })
            ->when($request->location,function($query)use($location){ $query->leftJoin('cp_location as loc','company.id','=','loc._id')->whereIn('loc.location',$location); })
            ->when($request->type,function($query)use($type){ $query->leftJoin('cp_type as rtp','company.id','=','rtp._id')->whereIn('rtp.type',$type); })
            ->when($request->service,function($query)use($service){ $query->leftJoin('cp_service as serv','company.id','=','serv._id')->whereIn('serv.service',$service); })
            ->when($request->rental,function($query)use($rental){ $query->leftJoin('cp_rental as ren','company.id','=','ren._id')->whereIn('ren.rental',$rental); })
    ...
    public function filtersHtml($cid)
    {
        ...
        if ($filters['location']->count() > 0) {
            ...
            foreach(@$filters['location']->get() as $k => $v){
        ...
        if ($filters['type']->count() > 0) {
            ...
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['type']->get() as $k => $v){
                $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
        if ($filters['service']->count() > 0) {
            ...
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['service']->get() as $k => $v){
                $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
        if ($filters['rental']->count() > 0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.rental").'</h5></div><div class="col-lg-10">';
            $html.='<ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['rental']->get() as $k => $v){
                $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
