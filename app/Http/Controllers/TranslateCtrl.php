...
class TranslateCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $counts = count($translate)+count($speciality)+count($status);
            $counts = ($urgent==1)?$counts+1:$counts+0;
            $counts = ($postpay==1)?$counts+1:$counts+0;

            $keyword = $request->keyword;
            
            $query = \App\Models\CompanyMd::
                where([
                    'company.public' => 1,
    ...
    public function filters($cid)
    {
        $lang = Session('lang');
        // $langP = ($lang=='th')?'th':'en';
        $translate = \App\Models\CpTranslateMd::select('ch.id as key',"ch.name_$lang as name")->leftJoin('translate as ch','cp_translate.translate','=','ch.id')->where(['cp_translate._id'=>@$cid]);
        $speciality = \App\Models\CpSpecialityMd::select('ch.id as key',"ch.name_$lang as name")->leftJoin('speciality as ch','cp_speciality.speciality','=','ch.id')->where(['cp_speciality._id'=>@$cid]);
        $status = \App\Models\CpStatusMd::select('ch.id as key',"ch.name_$lang as name")->leftJoin('status as ch','cp_status.status','=','ch.id')->where(['cp_status._id'=>$cid]);
        $urgent = \App\Models\CpUrgentMd::where('_id',$cid)->select('urgent')->first();
        $postpay = \App\Models\CpPostpayMd::where('_id',$cid)->select('postpay')->first();
        return [
            'translate' => $translate,
    ...
    public function filtersHtml($cid)
    {
        ...
        if ($filters['translate']->count() > 0) {
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.language").'</h5></div>';
            $html.='<div class="col-lg-8"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['translate']->get() as $v){
                $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
        if(@$filters['speciality']->count()>0){
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.speciality").'</h5></div>';
            $html.='<div class="col-lg-8"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['speciality']->get() as $v){
                $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
        if($filters['urgent']->urgent>0){
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-12"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.urgent").'</h5></div>';
            $html.='</div>';
        }
        if($filters['postpay']->postpay>0){
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-12"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.postpay").'</h5></div>';
            $html.='</div>';
        }
        if(@$filters['status']->count()>0){
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.status").'</h5></div>';
            $html.='<div class="col-lg-8"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['status']->get() as $v){
                $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
