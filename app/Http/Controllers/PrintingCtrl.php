...
class PrintingCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $counts = count($type)+count($minimum)+count($other)+count($location);
        ...
        ->when($request->type,function($query)use($type){
            $length = count($type);
            return $query->leftJoin('cp_printing as tp','company.id','=','tp._id')
                ->whereIn('tp.printing',$type)
    ...
    public function filtersHtml($cid)
    {
        ...
        if ($filters['minimum']->count()>0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.minimum").'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach(@$filters['minimum']->get() as $kv => $va){
                $html.='<div class="pix1uw-0 ggGntR">'.$va->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
        if ($filters['other']->count()>0) {
            $html.='<div class="row"><div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i>'.__("phrase.$this->industry.filter.other-service").'</h5></div><div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
