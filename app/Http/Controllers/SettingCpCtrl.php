...
class SettingCpCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $counts = count($location)+count($consulting)+count($service);
        ...
        ->when($request->consulting,function($query)use($consulting){ 
            ...
            return $query->leftJoin('cp_consulting as con','company.id','=','con._id')
                ->whereIn('con.consulting',$consulting)
                ->havingRaw('COUNT(con.id) >= ?',[$length]); 
        })
        ->when($request->service,function($query)use($service){ 
            $svLength = count($service);
                return $query->leftJoin('cp_service as sev','sev._id','=','company.id')
                    ->whereIn('sev.service',$service)
                    ->havingRaw('COUNT(sev.id) >= ?',[$svLength]);
        })
        ...
        $rows = $data->inRandomOrder()->limit(20)->get();
    ...
    public function store(Request $request)
    {
        ...
        switch ($request->step) {
            ...
            case 2:
                ...
                $CpConsultingMd = \App\Models\CpConsultingMd::class;
                if (is_countable(@$request->consulting)>0) {
                    foreach ($request->consulting as $con) {
                        $new['consulting'] = new $CpConsultingMd;
                        $new['consulting']->_id = $get->id;
                        $new['consulting']->consulting = $con;
                        $new['consulting']->created = date('Y-m-d H:i:s');   
                        $new['consulting']->save();
                    }
                }
                if($get->save())
    ...
    public function informationUpdate(Request $request,$id=null)
    {
        try {
            ...
            if($request->location){
                foreach($request->location as $lo) {
                    if ($CpLocationMd::where(['_id'=>$get->id,'location'=>$lo])->count() == 0) {
                        $newL = new $CpLocationMd;
                        $newL->_id = $get->id;
                        $newL->location = $lo;
                        $newL->created = date('Y-m-d H:i:s');
                        $newL->save();
                }}
            ...
            if($request->condition){
                foreach($request->condition as $kin => $in) {
                    if($CpConditionMd::where(['_id'=>$get->id,'condition'=>$in])->count() == 0){
                        $CpConditionMd::insert(['_id'=>$get->id,'condition'=>$in,'created'=>date('Y-m-d H:i:s')]);
                    };
                }
    ...
    public function filters($cid)
    {
        $lang = Session('lang');
        $langP = ($lang=='th')?'th':'en';
        $location = \App\Models\CpLocationMd::select("pro.province_name_$langP as name")->leftJoin('provinces as pro','cp_location.location','=','pro.province_id')->where(['cp_location._id'=>@$cid]);
        $consulting = \App\Models\CpConsultingMd::select('ch.id',"ch.name_$lang as name")->leftJoin('consulting as ch','cp_consulting.consulting','=','ch.id')->where(['cp_consulting._id'=>@$cid]);
        $service = \App\Models\CpServiceMd::select('ch.id',"ch.name_$lang as name")->leftJoin('choice as ch','cp_service.service','=','ch.key')->where(['cp_service._id'=>$cid,'ch.type'=>'setting-service']);
        return [
            'location' => $location,
    ...
    public function filtersHtml($cid)
    {
        ...
        if ($filters['location']->count() > 0) {
            ...
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.location").'</h5></div>';
            $html.='<div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach($filters['location']->get() as $v){
                $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
        if ($filters['consulting']->count() > 0) {
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-3"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.consulting").'</h5></div>';
            $html.='<div class="col-lg-9"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach($filters['consulting']->get() as $v){
                $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
        if ($filters['service']->count() > 0) {
            $html.='<div class="row mb-2">';
            $html.='<div class="col-lg-2"><h5 class="title bold"><i class="icofont-verification-check text-success"></i> '.__("phrase.$this->industry.filter.service").'</h5></div>';
            $html.='<div class="col-lg-10"><ul class="ey7ls2-0"><li class="fa-Dycg"><div class="bDELcg">';
            foreach($filters['service']->get() as $v){
                $html.='<div class="pix1uw-0 ggGntR">'.$v->name.'</div>';
            }
            $html.='</div></li></ul></div></div>';
        }
    ...
    public function cp($id=null)
    {
        ...
        $consulting = \App\Models\CpConsultingMd::select('ch.id',"ch.name_$lang as name")->leftJoin('consulting as ch','cp_consulting.consulting','=','ch.id')->where(['cp_consulting._id'=>@$cp->id]);
        $service = \App\Models\CpServiceMd::select('ch.id',"ch.name_$lang as name")->leftJoin('choice as ch','cp_service.service','=','ch.key')->where(['cp_service._id'=>$cp->id,'ch.type'=>'setting-service']);
