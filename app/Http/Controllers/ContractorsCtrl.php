...
class ContractorsCtrl extends Controller
{
    ...
    public function index(Request $request)
    {
        // try {
            ...
            $counts = count($service) + count($other) + count($location);
            $online = 0;
            ...
            ->when($request->service,function($query)use($service){
                ...
                return $query->join('cp_service as sv','company.id','=','sv._id')
                    ->whereIn('sv.service',$service)
                    ->havingRaw('COUNT(sv.id) >= ? ',[$length]);
            })
            ->when($request->other,function($query)use($other){
                $length = count($other);
                return $query->join('cp_other as ot','company.id','=','ot._id')
