...
$('#contract').hunterPopup({
    width: popUp.contract.width,
    title: $('#contract').attr('title'),
    content: $('#tableSecond'),
    placement: popUp.contract.placement,
    resetBtn: '.reset-all-filters',
    event:function(){
        box = $('#contract');
        var second = {id:[],text:[]};
...
$('#location').hunterPopup({
    ...
    event:function(){
        ...
        $('.clear-list').click(function(){
            ...
            $('.select-all:checked').prop('checked',false);
            box.html(box.attr('title'));
            box.removeClass(color);
        })
        $('.list-filter').click(function(){
            const t = $(this);
            $(this).addClass(color);
            $('.list-filter').not($(this)).removeClass(color);
            $('.sub-'+t.attr('province')).show();
            $('.-sub').not($('.sub-'+t.attr('province'))).hide();
            
        });
        $('.select-all').click(function(){
            const t = $(this);
            check = (t.is(':checked'))?true:false;
            $('.sub-'+t.val()).children().prop('checked',check);
            second = {id:[],text:[]};
            adjust(box);
        })
    }
...
{
    /*===================================*/
    box = {first:$('#type'),second:$('#location'),third:$('#third'),fourth:$('#fourth'),fifth:$('#fifth'),sixth:$('#sixth')}
