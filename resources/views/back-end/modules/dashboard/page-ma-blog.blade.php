...
<div class="card">
    <div class="card-body">
        <div class="row">
            ...
            @php
                if (Request::get('keyword')) {
                    $filteredRows = array_filter($rows, function ($obj) {
                        return $obj->stClick > 0 && (strpos($obj->company_name, Request::get('keyword')) || strpos($obj->customer_name, Request::get('keyword')) || strpos($obj->email, Request::get('keyword')) || strpos($obj->department, Request::get('keyword')) || strpos($obj->companyName, Request::get('keyword')) || (Request::get('keyword') == $obj->company_name || Request::get('keyword') == $obj->customer_name || Request::get('keyword') == $obj->email || Request::get('keyword') == $obj->department || Request::get('keyword') == $obj->companyName));
                    });
                } else {
                    $filteredRows = array_filter($rows, function ($obj) {
            ...
            @endphp
            <div class="col-lg-12">
                <h5 class="text-center">Ranking by Blog {{ $count }} </h5>
            </div>
            <div class="col-lg-12">
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach ($filteredRows as $index => $val)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $val->customer_name }} - {{ $val->company_name }}</td>
                                    <td>{{ $val->email }}</td>
                                    <td>{{ $val->department }}</td>
                                    <td>[<strong>{{ $val->companyId }}</strong>]
                                        {{ $val->companyName }}
                                    </td>
                                    <td>
                                        @if ($val->ips->count() > 0)
...
<script>
    ...
    const ClicksData = (ip, id) => {
        const data = $.ajax({
            method: 'get',
            url: 'webpanel/web-traffic/get-clicks-blog',
            async: false,
            data: {
                ip: ip,
                id: id
